...
let parisController: ParisController;

   init(parisController: ParisController) {
		self.parisController = parisController;
	...
	required init(coder aDecoder: NSCoder) {
		parisController = ParisController(title: "");
		super.init(coder: aDecoder);
	}
	
	override func touchesEnded(touches: NSSet, withEvent event: UIEvent) {
		let mapController: MapController = parisController.navigationController! as MapController;
		mapController.next2();	//Advance east to the next station.
	}
	...
	override func layoutSubviews() {
		bounds.origin = CGPointMake(
			bounds.origin.x,
			-parisController.topLayoutGuide.length);
	}
	...
	override func drawRect(rect: CGRect) {
		...
		var s: String = "\(parisController.title!)\n"
		...
		let image: UIImage? = UIImage(named: "2.jpg");
		if image == nil {
